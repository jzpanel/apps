...
所有应用必须挂在 `panel_network` 上。这样：
- 容器之间能通过容器名互相访问（如 phpMyAdmin 连接 `panel_mysql`）
- 与面板进程在同一网络下，便于反代
...
3. **多版本应用**：[`php_83/`](../php_83/) — version_key_template 模式
4. **依赖其他服务的工具**：[`phpmyadmin/`](../phpmyadmin/) — compose_vars 拉 mysql 凭据
5. **多容器复杂应用**：[`wordpress/`](../wordpress/) — db_init + service_dependencies

---

## 文档维护

本文档版本：v1.0（2026-05-25）
